    ...
    <div className="space-y-4">
      <div className="flex justify-between items-start gap-3">
        ...
        <div className="flex justify-end space-x-2">
          {hide ? (
            <PanelTopOpen onClick={() => setHide(!hide)} className="text-muted-foreground" />
          ) : (
            <PanelTopClose onClick={() => setHide(!hide)} className="text-muted-foreground" />
          )}
        </div>
      </div>
      {hide ? (
        <div className="flex gap-2">内容已隐藏，点击 <PanelTopOpen /> 显示</div>
      ) : (
        <>
